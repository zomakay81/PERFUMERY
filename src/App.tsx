...
import { Menu, X, LayoutDashboard, FlaskConical, Package, BookImage, Users, FileText, Sparkles, ShoppingCart, Truck, Settings, History, Wallet, Undo2, Redo2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden pt-[72px] md:pt-0 relative">
        ...
        <div className="absolute top-6 right-12 z-30 flex items-center gap-2">
          <button 
            disabled={!canUndo}
            onClick={undo}
            className={`p-2.5 rounded-xl border border-white/5 backdrop-blur-md transition-all flex items-center gap-2 font-bold text-[10px] uppercase tracking-widest ${
              canUndo ? 'bg-slate-900/50 text-purple-400 hover:bg-purple-500/10' : 'bg-slate-950/20 text-slate-600 opacity-50 cursor-not-allowed'
            }`}
          >
          ...
          <button 
            ...
            onClick={redo}
            className={`p-2.5 rounded-xl border border-white/5 backdrop-blur-md transition-all flex items-center gap-2 font-bold text-[10px] uppercase tracking-widest ${
              canRedo ? 'bg-slate-900/50 text-blue-400 hover:bg-blue-500/10' : 'bg-slate-950/20 text-slate-600 opacity-50 cursor-not-allowed'
            }`}
          >
            <Redo2 size={16} /> Ripristina
          </button>
        </div>
